...
7. [Presentation Layer — `apps/app_macos/lib`](#7-presentation-layer)
...
project_root/
├── apps/
│   ├── app_macos/              # Application Flutter mobile
│   │   ├── lib/
...
│   │   │   ├── generated/       # Fichiers generes
│   │   │   │   ├── assets/      # Fichiers generes flutter_gen (assets.gen.dart)
│   │   │   │   └── translations/# Fichiers generes easy_localization (locale_keys.g.dart)
...
```
lib/features/
├── live_transcription/       # Enregistrement et transcription en direct (live session)
│   └── presentation/
│       └── screens/
│           └── live/
...
│               └── widgets/
├── transcript_viewer/        # Consultation d'un transcript existant
│   └── presentation/
│       └── screens/
├── history/                  # Liste et detail des sessions passees
│   └── presentation/
│       └── screens/
│           ├── list/
│           │   └── widgets/
│           └── detail/
├── onboarding/               # Setup initial (Ollama, etc.)
│   └── presentation/
...
└── shared/
    └── presentation/
        ├── screens/          # Ecrans communs (shell, navigation...)
        └── widgets/          # Widgets reutilisables cross-feature
```

### Pattern de screen (triade ecran)

Chaque ecran a la meme structure interne :

```
{screen_name}/
├── {screen_name}.screen.dart       # Widget UI (@RoutePage ConsumerStatefulWidget)
├── {screen_name}.view_model.dart   # Logique (@riverpod class)
├── {screen_name}.view_model.g.dart # GENERE
├── {screen_name}.state.dart        # Etat immutable (Freezed)
├── {screen_name}.state.freezed.dart# GENERE
└── widgets/                        # Widgets specifiques a cet ecran
```

### Regle de la triade ecran

Chaque ecran est TOUJOURS compose de **3 fichiers minimum** :
...
- Ils vivent dans `core_domain/lib/domain/services/` s'ils sont partageables, ou dans `app_macos/lib/application/services/` s'ils sont specifiques a l'app mobile.
...
- `lib/generated/assets/assets.gen.dart` — references type-safe aux assets
...
- `lib/generated/translations/locale_keys.g.dart`
...
### Phase 3 : Providers (apps/app_macos/lib/core/providers)
...
### Phase 4 : Presentation (apps/app_macos/lib/features)
...
apps/app_macos/lib/core/providers/
...
apps/app_macos/lib/features/client/presentation/screens/appointments/
...
├── Orchestration multi-use-case ? → Service (core_domain/services/ ou app_macos/application/services/)
